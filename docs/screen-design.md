...
| 5 | 問い合わせ詳細/編集モーダル | 内容確認、ステータス/優先度/担当者の変更、対応履歴・コメントの閲覧/投稿 | スタッフ | ○(PC限定の操作なし) |
| 6 | 検索結果画面 | 検索ボックスで氏名・メール・内容を横断検索した結果を一覧表示する | スタッフ | ○(ドラッグ&ドロップ非対応の画面のため、PC/スマホで操作差なし) |
...
- 画面幅が狭い場合(lg=1024px未満)、3カラムを横並びではなく縦に積んで表示する。未対応→対応中→完了の順に、下にスクロールして続けて閲覧する(プルダウン等でのカラム切り替えは無い)
