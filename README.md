...
ステータス/優先度/担当者の変更、コメント投稿、完了への移行までの流れ。

https://github.com/user-attachments/assets/496b1c4f-ea44-49b1-8b1e-3622e32591c9

### 検索機能

完了から30日経過すると一覧から見えなくなる問い合わせを、名前・メール・問い合わせ内容から横断検索し、ボード画面に戻るまでの流れ。

https://github.com/user-attachments/assets/9dad331c-6f51-45b8-8528-a6f1cf7656d9
...
- **問い合わせボード**:未対応/対応中/完了のカンバン形式一覧。ドラッグ&ドロップまたはプルダウンでのステータス変更、優先度順/受付日時順ソート、未対応のまま24時間経過したカードの強調表示、20件ごとのページネーション(「もっと見る」で追加読み込み)。完了は一覧が際限なく溜まらないよう直近30日以内のもののみ表示(それより古いものは検索機能から探せる)
- **問い合わせ詳細**:ステータス・優先度・担当者の変更、対応履歴とコメントのやり取り(変更は自動でコメントとして記録)
- **検索機能**:名前・メール・問い合わせ内容を横断検索。完了から30日経過してボード一覧から見えなくなった問い合わせも検索対象
...
| GET | `/inquiries/search` | 問い合わせ検索(名前・メール・内容の部分一致、全期間・全ステータス横断) | 必須 |
